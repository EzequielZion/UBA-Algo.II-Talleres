...
TEST(diccionario, dicc_string_bool) {
    ...
    ASSERT_FALSE(d.def("santi"));
    ASSERT_FALSE(d.def("ezequiel"));
    ASSERT_FALSE(d.def("algo2"));
    d.definir("santi", true);
    ASSERT_TRUE(d.def("santi"));
    ASSERT_FALSE(d.def("ezequiel"));
    ASSERT_FALSE(d.def("algo2"));
    ASSERT_EQ(d.obtener("santi"), true);
    d.definir("ezequiel", true);
    ASSERT_TRUE(d.def("santi"));
    ASSERT_TRUE(d.def("ezequiel"));
    ASSERT_FALSE(d.def("algo2"));
    ASSERT_EQ(d.obtener("santi"), true);
    ASSERT_EQ(d.obtener("ezequiel"), true);
    d.definir("santi", false);
    ASSERT_TRUE(d.def("santi"));
    ASSERT_TRUE(d.def("ezequiel"));
    ASSERT_FALSE(d.def("algo2"));
    ASSERT_EQ(d.obtener("santi"), false);
    ASSERT_EQ(d.obtener("ezequiel"), true);
}
#endif
